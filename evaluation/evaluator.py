...
def main():
    ...
    txt_path = args.sample_batch.replace('.npz', '_results.txt')
    os.makedirs(os.path.dirname(txt_path), exist_ok=True)
    print(f'Metrics are saved in {txt_path}')
    ...
    with open(txt_path, 'a') as file:
        file.write(f"\n[{current_time}]: {args.ref_batch} and {args.sample_batch}\n")
        print("Computing evaluations...")

        fid = sample_stats.frechet_distance(ref_stats)
        print("FID:", fid)
        file.write(f"FID:{fid:.4f}\n")

        inception_score = evaluator.compute_inception_score(sample_acts[0])
        print("Inception Score:", inception_score)
        file.write(f"Inception Score:{inception_score:.4f}\n")

        sfid = sample_stats_spatial.frechet_distance(ref_stats_spatial)
        print("sFID:", sfid)
        file.write(f"sFID:{sfid:.4f}\n")
